...
#include "znomd/zerocoin.h"
#include "znomd/znomdmodule.h"
...
#if defined(NDEBUG)
#error "NOMADIC cannot be compiled without assertions."
#endif
...
/** Fees smaller than this (in unomd) are considered zero fee (for relaying, mining and transaction creation)
                              ...
                              std::vector<COutPoint>& coins_to_uncache)
{
...

    {
        ...
        if (hasZcSpendInputs) {
            ...
            // Reject legacy zNOMD mints
            if (!Params().IsRegTestNet() && tx.HasZerocoinMintOutputs())
                return state.Invalid(error("%s : tried to include zNOMD mint output in tx %s",
...
CAmount GetBlockValue(int nHeight)
{
    ...
    // Testnet high-inflation blocks [2, 200] with value 250k NOMD
    ...
    else if (nHeight <= 360001) {
        ret = COIN * 750;
    }
    else if (nHeight <= 660001) {
        ret = COIN * 550;
    }
    else if (nHeight <= 965145) {
        ret = COIN * 350;
    }
    else if (nHeight <= 1299932) {
        ret = COIN * 250;
    }
    else if (nHeight <= 2099932) {
        ret = COIN * 150;
    }
    else if (nHeight <= 5499932) {
        ret = COIN * 50;
    }
    else if (nHeight <= 9899932) {
        ret = COIN * 25;
    }
	else if (nHeight <= 98380000) {
        ret = COIN * 11;
    }
    else {
        ret = COIN * 0;
        ...
        for (const CTxIn& in : tx.vin) {
            ...
            if (in.IsZerocoinSpend() || isPublicSpend) {
                ...
                if (isPublicSpend) {
                    ...
                    if (!ZNOMDModule::ParseZerocoinPublicSpend(in, tx, state, publicSpend)){
...
{
    util::ThreadRename("nomadic-scriptch");
        ...
        if (tx.HasZerocoinSpendInputs()) {
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (isPublicSpend) {
                    ...
                    PublicCoinSpend publicSpend(params);
                    if (!ZNOMDModule::ParseZerocoinPublicSpend(txIn, tx, state, publicSpend)){
                        return false;
                    }
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const auto& txIn : block.vtx) {
        ...
        // double check that there are no double spent zNOMD spends in this block
        if (tx.HasZerocoinSpendInputs()) {
            for (const CTxIn& txIn : tx.vin) {
                bool isPublicSpend = txIn.IsZerocoinPublicSpend();
                if (txIn.IsZerocoinSpend() || isPublicSpend) {
                    ...
                    if (std::count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zNOMD serial %s in block\n Block: %s",
...
bool CheckWork(const CBlock block, CBlockIndex* const pindexPrev)
{
    ...
    if (block.nBits != nBitsRequired) {
        // Nomadic Specific reference to the block with the wrong threshold was used.
        const Consensus::Params& consensus = Params().GetConsensus();
        if ((block.nTime == (uint32_t) consensus.nNomadicBadBlockTime) &&
                (block.nBits == (uint32_t) consensus.nNomadicBadBlockBits)) {
            // accept NOMADIC block minted with incorrect proof of work threshold
            return true;
        }
...
bool AcceptBlock(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp, bool fAlreadyCheckedBlock)
{
    ...
    if (isPoS) {
        ...
        // Inputs
        std::vector<CTxIn> nomdInputs;
        std::vector<CTxIn> zNOMDInputs;

        for (const CTxIn& stakeIn : stakeTxIn.vin) {
            if(stakeIn.IsZerocoinSpend()){
                zNOMDInputs.push_back(stakeIn);
            }else{
                nomdInputs.push_back(stakeIn);
            }
        }
        const bool hasNOMDInputs = !nomdInputs.empty();
        const bool hasZNOMDInputs = !zNOMDInputs.empty();
        ...
        for (const auto& txIn : block.vtx) {
            ...
            for (const CTxIn& in: tx.vin) {
                ...
                if(hasNOMDInputs) {
                    ...
                    for (const CTxIn& nomdIn : nomdInputs)
                        if(nomdIn.prevout == in.prevout)
        ...
        if (isBlockFromFork) {
            ...
            // Now that this loop if completed. Check if we have zNOMD inputs.
            if(hasZNOMDInputs) {
                for (const CTxIn& zNomdInput : zNOMDInputs) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(zNomdInput);
                    ...
                    if (!ContextualCheckZerocoinSpendNoSerialCheck(stakeTxIn, &spend, pindex->nHeight, UINT256_ZERO))
                        return state.DoS(100,error("%s: forked chain ContextualCheckZerocoinSpend failed for tx %s", __func__,
                                                   stakeTxIn.GetHash().GetHex()), REJECT_INVALID, "bad-txns-invalid-znomd");
