...
Does not use functions of NomadicTier2TestFramework as we don't want to send
...
class MasternodePingTest(NomadicTestFramework):
    ...
    def run_test(self):
        ...
        collateralTxId = miner.sendtoaddress(mnAddress, Decimal('100000'))
        miner.generate(2)
        sync_blocks(self.nodes)
        time.sleep(1)
        collateral_rawTx = owner.getrawtransaction(collateralTxId, 1)
        assert_equal(owner.getbalance(), Decimal('100000'))
