...
CNomdStake* CNomdStake::NewNomdStake(const CTxIn& txin)
{
    if (txin.IsZerocoinSpend()) {
        error("%s: unable to initialize CNomdStake from zerocoin spend", __func__);
        return nullptr;
    }
    ...
    return new CNomdStake(txPrev.vout[txin.prevout.n],
                         txin.prevout,
                         pindexFrom);
}

bool CNomdStake::GetTxOutFrom(CTxOut& out) const
{
    out = outputFrom;
    return true;
}

bool CNomdStake::CreateTxIn(CWallet* pwallet, CTxIn& txIn, uint256 hashTxOut)
{
    txIn = CTxIn(outpointFrom.hash, outpointFrom.n);
    return true;
}

CAmount CNomdStake::GetValue() const
{
    return outputFrom.nValue;
}

bool CNomdStake::CreateTxOuts(CWallet* pwallet, std::vector<CTxOut>& vout, CAmount nTotal, const bool onlyP2PK)
{
...
CDataStream CNomdStake::GetUniqueness() const
{
    //The unique identifier for a NOMD stake is the outpoint
...
const CBlockIndex* CNomdStake::GetIndexFrom() const
{
    // Sanity check, pindexFrom is set on the constructor.
    if (!pindexFrom) throw std::runtime_error("CNomdStake: uninitialized pindexFrom");
    return pindexFrom;
}

// Verify stake contextual checks
bool CNomdStake::ContextCheck(int nHeight, uint32_t nTime)
{
