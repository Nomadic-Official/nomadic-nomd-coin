...
void CoinControlDialog::updateLabels()
{
    ...
    ui->labelTitle->setText(fSelectTransparent ?
            "Select NOMD Outputs to Spend" :
            "Select Shielded NOMD to Spend");
    ...
    int nDisplayUnit = BitcoinUnits::NOMD;
                                          ...
                                          const bool isChange)
{
    ...
    // if listMode or change => show NOMADIC address. In tree mode, address is not shown again for direct wallet address outputs
    if (!treeMode) {
