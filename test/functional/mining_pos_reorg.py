...
class ReorgStakeTest(NomadicTestFramework):
    ...
    def check_money_supply(self, expected_nomd):
        # verify that nodes have the expected NOMD supply
        nomd_supply = [self.nodes[i].getsupplyinfo(True)['transparentsupply']
                      for i in range(self.num_nodes)]
        assert_equal(nomd_supply, [DecimalAmt(expected_nomd)] * self.num_nodes)
    ...
    def run_test(self):
        ...
        # Verify that NOMD supply was properly updated after the reorgs
        self.log.info("Check NOMD supply...")
