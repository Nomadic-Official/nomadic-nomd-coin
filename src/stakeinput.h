...
#ifndef NOMADIC_STAKEINPUT_H
#define NOMADIC_STAKEINPUT_H
...
public:
    ...
    virtual bool IsZNOMD() const = 0;
    virtual CDataStream GetUniqueness() const = 0;
    virtual bool ContextCheck(int nHeight, uint32_t nTime) = 0;
};


class CNomdStake : public CStakeInput
{
private:
...
public:
    CNomdStake(const CTxOut& _from, const COutPoint& _outPointFrom, const CBlockIndex* _pindexFrom) :
            CStakeInput(_pindexFrom), outputFrom(_from), outpointFrom(_outPointFrom) {}

    static CNomdStake* NewNomdStake(const CTxIn& txin);
    ...
    bool IsZNOMD() const override { return false; }
    bool ContextCheck(int nHeight, uint32_t nTime) override;
};


#endif //NOMADIC_STAKEINPUT_H
