...
*Setup instructions for a gitian build of NOMADIC Core using a VM or physical system.*

Gitian is the deterministic build process that is used to build the NOMADIC
...
to the NOMADIC GitHub release page.
...
- [Building NOMADIC Core](#building-nomadic-core)
...
```bash
gpg --output ${VERSION}-linux/${NAME}/nomadic-linux-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-linux/$NAME/nomadic-linux-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-osx-unsigned/$NAME/nomadic-osx-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-osx-unsigned/$NAME/nomadic-osx-${VERSION%\.*}-build.assert
gpg --output ${VERSION}-win-unsigned/$NAME/nomadic-win-${VERSION%\.*}-build.assert.sig --detach-sign ${VERSION}-win-unsigned/$NAME/nomadic-win-${VERSION%\.*}-build.assert
```

Uploading Signatures
--------------------
Make a Pull Request (both the `.assert` and `.assert.sig` files) to the
[gitian.sigs](https://github.com/nomadic-project/gitian.sigs/) repository:
...
You can also mail the files to Fuzzbawls (fuzzbawls@nomadic.org) and he will commit them.

```bash
gpg --detach-sign ${VERSION}-linux/${NAME}/nomadic-linux-*-build.assert
gpg --detach-sign ${VERSION}-win-unsigned/${NAME}/nomadic-win-*-build.assert
gpg --detach-sign ${VERSION}-osx-unsigned/${NAME}/nomadic-osx-*-build.assert
```
