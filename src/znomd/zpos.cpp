...
bool CLegacyZNomdStake::InitFromTxIn(const CTxIn& txin)
{
    // Construct the stakeinput object
    if (!txin.IsZerocoinSpend())
        return error("%s: unable to initialize CLegacyZNomdStake from non zc-spend", __func__);
    ...
    *this = CLegacyZNomdStake(spend);

    // Find the pindex with the accumulator checksum
    if (!GetIndexFrom())
        return error("%s : Failed to find the block index for znomd stake origin", __func__);

    // All good
    return true;
}

CLegacyZNomdStake::CLegacyZNomdStake(const libzerocoin::CoinSpend& spend) : CStakeInput(nullptr)
{
...
const CBlockIndex* CLegacyZNomdStake::GetIndexFrom() const
{
...
CAmount CLegacyZNomdStake::GetValue() const
{
    return denom * COIN;
}

CDataStream CLegacyZNomdStake::GetUniqueness() const
{
...
bool CLegacyZNomdStake::ContextCheck(int nHeight, uint32_t nTime)
{
    const Consensus::Params& consensus = Params().GetConsensus();
    if (!consensus.NetworkUpgradeActive(nHeight, Consensus::UPGRADE_ZC_V2) || nHeight >= consensus.height_last_ZC_AccumCheckpoint)
        return error("%s : zNOMD stake block: height %d outside range", __func__, nHeight);
