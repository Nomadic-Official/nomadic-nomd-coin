...
#include "qt/nomadic/forms/ui_snackbar.h"
#include "qt/nomadic/qtutils.h"
#include <QTimer>


SnackBar::SnackBar(NOMADICGUI* _window, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::SnackBar),
    window(_window),
    timeout(MIN_TIMEOUT)
{
    ...
    if (window)
        connect(window, &NOMADICGUI::windowResizeEvent, this, &SnackBar::windowResizeEvent);
    else {
