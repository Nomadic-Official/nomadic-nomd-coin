...
#include "qt/nomadic/settings/forms/ui_settingsconsolewidget.h"
...
#include "qt/nomadic/settings/moc_settingsconsolewidget.cpp"
...
SettingsConsoleWidget::SettingsConsoleWidget(NOMADICGUI* _window, QWidget *parent) :
...
void SettingsConsoleWidget::clear(bool clearHistory)
{
    ...
    messageInternal(CMD_REPLY, (tr("Welcome to the NOMADIC RPC console.") + "<br>" +
...
static bool PotentiallyDangerousCommand(const QString& cmd)
{
    ...
    if (cmd.size() >= 13 && cmd.leftRef(11) == "dumpprivkey") {
        // valid NOMADIC Transparent Address
        std::vector<std::string> args;
