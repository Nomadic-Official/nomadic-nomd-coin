...
#include "qt/nomadic/forms/ui_addresseswidget.h"
#include "qt/nomadic/addresslabelrow.h"
#include "qt/nomadic/addnewaddressdialog.h"
#include "qt/nomadic/tooltipmenu.h"
...
AddressesWidget::AddressesWidget(NOMADICGUI* parent) :
    PWidget(parent),
...
void AddressesWidget::onStoreContactClicked()
{
    if (walletModel) {
        ...
        auto nomdAdd = Standard::DecodeDestination(address.toUtf8().constData(), isStakingAddress);

        if (!Standard::IsValidDestination(nomdAdd)) {
        ...
        bool isShielded = walletModel->IsShieldedDestination(nomdAdd);
        if (walletModel->updateAddressBookLabels(nomdAdd, label.toUtf8().constData(),
