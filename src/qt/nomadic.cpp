...
#include "config/nomadic-config.h"
#endif
...
#include "qt/nomadic/splash.h"
#include "qt/nomadic/welcomecontentwidget.h"
...
{
    return QCoreApplication::translate("nomadic-core", psz).toStdString();
}
...
static void initTranslations(QTranslator& qtTranslatorBase, QTranslator& qtTranslator, QTranslator& translatorBase, QTranslator& translator, bool forceLangFromSettings = false)
{
    ...
    // Load e.g. bitcoin_de.qm (shortcut "de" needs to be defined in nomadic.qrc)
    if (translatorBase.load(lang, ":/translations/"))
        QApplication::installTranslator(&translatorBase);

    // Load e.g. bitcoin_de_DE.qm (shortcut "de_DE" needs to be defined in nomadic.qrc)
...
/** Class encapsulating NOMADIC Core startup and shutdown.
...
/** Main NOMADIC application object */
...
public:
    ...
    /// Get window identifier of QMainWindow (NOMADICGUI)
...
{
    window = new NOMADICGUI(networkStyle, 0);

    pollShutdownTimer = new QTimer(window);
    connect(pollShutdownTimer, &QTimer::timeout, window, &NOMADICGUI::detectShutdown);
...
void BitcoinApplication::startThread()
{
    ...
    connect(window, &NOMADICGUI::requestedRestart, executor, &BitcoinCore::restart);
...
void BitcoinApplication::initializeResult(int retval)
{
    ...
    if (retval) {
        ...
        if (pwalletMain) {
            ...
            window->addWallet(NOMADICGUI::DEFAULT_WALLET, walletModel);
            window->setCurrentWallet(NOMADICGUI::DEFAULT_WALLET);
        ...
        // NOMADIC: URIs or payment requests:
        //connect(paymentServer, &PaymentServer::receivedPaymentRequest, window, &NOMADICGUI::handlePaymentRequest);
        connect(window, &NOMADICGUI::receivedURI, paymentServer, &PaymentServer::handleURIOrFile);
...
{
    QMessageBox::critical(0, "Runaway exception", QObject::tr("A fatal error occurred. NOMADIC can no longer continue safely and will quit.") + QString("\n\n") + message);
    ::exit(1);
}
...
/// 2. Basic Qt initialization (not dependent on parameters or configuration)
    Q_INIT_RESOURCE(nomadic_locale);
    Q_INIT_RESOURCE(nomadic);
    ...
    /// 6. Determine availability of data directory and parse nomadic.conf
    /// - Do not call GetDataDir(true) before this step finishes
    if (!fs::is_directory(GetDataDir(false))) {
        QMessageBox::critical(0, QObject::tr("NOMADIC Core"),
    ...
    if (!SelectParamsFromCommandLine()) {
        QMessageBox::critical(0, QObject::tr("NOMADIC Core"), QObject::tr("Error: Invalid combination of -regtest and -testnet."));
        return 1;
    }
#ifdef ENABLE_WALLET
    ...
    // nomadic: links repeatedly have their payment requests routed to this process:
...
#if defined(Q_OS_WIN)
        WinShutdownMonitor::registerShutdownBlockReason(QObject::tr("NOMADIC Core didn't yet exit safely..."), (HWND)app.getMainWinId());
#endif
        app.exec();
        app.requestShutdown();
        app.exec();
