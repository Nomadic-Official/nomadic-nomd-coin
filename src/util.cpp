...
const char * const NOMADIC_CONF_FILENAME = "nomadic.conf";
const char * const NOMADIC_PID_FILENAME = "nomadic.pid";
const char * const NOMADIC_MASTERNODE_CONF_FILENAME = "masternode.conf";


// NOMADIC only features
// Masternode
...
{
// Windows < Vista: C:\Documents and Settings\Username\Application Data\NOMADIC
// Windows >= Vista: C:\Users\Username\AppData\Roaming\NOMADIC
// Mac: ~/Library/Application Support/NOMADIC
// Unix: ~/.nomadic
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NOMADIC";
#else
    fs::path pathRet;
...
static fs::path ZC_GetBaseParamsDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\NOMADICParams
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\NOMADICParams
    // Mac: ~/Library/Application Support/NOMADICParams
    // Unix: ~/.nomadic-params
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "NOMADICParams";
#else
    fs::path pathRet;
    char* pszHome = getenv("HOME");
    if (pszHome == NULL || strlen(pszHome) == 0)
    ...
    pathRet /= "Library/Application Support";
    TryCreateDirectory(pathRet);
    return pathRet / "NOMADICParams";
#else
    // Unix
    return pathRet / ".nomadic-params";
#endif
#endif
}
...
void initZKSNARKS()
{
    ...
    if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
        ...
        sapling_spend = "/usr/share/nomadic/sapling-spend.params";
        sapling_output = "/usr/share/nomadic/sapling-output.params";
        if (fs::exists(sapling_spend) && fs::exists(sapling_output)) {
            fParamsFound = true;
        } else {
            // Linux fallback for local installs
            sapling_spend = "/usr/local/share/nomadic/sapling-spend.params";
            sapling_output = "/usr/local/share/nomadic/sapling-output.params";
        }
#endif
        if (fs::exists(sapling_spend) && fs::exists(sapling_output))
            fParamsFound = true;
...
{
    fs::path pathConfigFile(gArgs.GetArg("-conf", NOMADIC_CONF_FILENAME));
    return AbsPathForConfigVal(pathConfigFile, false);
}

fs::path GetMasternodeConfigFile()
{
    fs::path pathConfigFile(gArgs.GetArg("-mnconf", NOMADIC_MASTERNODE_CONF_FILENAME));
...
void ArgsManager::ReadConfigFile()
{
    ...
    if (!streamConfig.good()) {
        // Create empty nomadic.conf if it does not exist
...

    {
        ...
        for (boost::program_options::detail::config_file_iterator it(streamConfig, setOptions), end; it != end; ++it) {
            // Don't overwrite existing settings so command line settings override nomadic.conf
...
fs::path GetPidFile()
{
    fs::path pathPidFile(gArgs.GetArg("-pid", NOMADIC_PID_FILENAME));
