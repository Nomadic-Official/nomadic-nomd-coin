...
if [ -n "${1:-}" ]; then
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        PARAMS_DIR="$HOME/Library/Application Support/NOMADICParams"
    else
        PARAMS_DIR="$HOME/.nomadic-params"
    fi
fi
...
Failed to fetch the NOMADIC zkSNARK parameters!
...
function main() {
    ...
    cat <<EOF
NOMADIC - fetch-params.sh

This script will fetch the NOMADIC zkSNARK parameters and verify their
...
EOF
    ...
    then
        ...
        cat >> "$README_PATH" <<EOF
This directory stores common NOMADIC zkSNARK parameters. Note that it is
