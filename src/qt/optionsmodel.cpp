...
void OptionsModel::setDisplayDefaultOptions(QSettings& settings, bool reset)
{
    if (!settings.contains("nDisplayUnit") || reset)
        settings.setValue("nDisplayUnit", BitcoinUnits::NOMD);
    ...
    if (settings.contains("nAnonymizeNomadicAmount") || reset)
        gArgs.SoftSetArg("-anonymizenomadicamount", settings.value("nAnonymizeNomadicAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in nomadic.cpp during shotdown to also remove the window positions
