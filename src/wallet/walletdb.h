...
public:
    ...
    bool WriteZNOMDSeed(const uint256& hashSeed, const std::vector<unsigned char>& seed);
    bool ReadZNOMDSeed(const uint256& hashSeed, std::vector<unsigned char>& seed);
    bool ReadZNOMDSeed_deprecated(uint256& seed);
    bool EraseZNOMDSeed();
    bool EraseZNOMDSeed_deprecated();

    bool WriteZNOMDCount(const uint32_t& nCount);
    bool ReadZNOMDCount(uint32_t& nCount);
