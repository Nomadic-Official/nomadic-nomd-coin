...
#include "qt/nomadic/dashboardwidget.h"
#include "qt/nomadic/forms/ui_dashboardwidget.h"
#include "qt/nomadic/sendconfirmdialog.h"
#include "qt/nomadic/txrow.h"
...
DashboardWidget::DashboardWidget(NOMADICGUI* parent) :
    PWidget(parent),
    ui(new Ui::DashboardWidget)
{
    ...
    setCssProperty(ui->labelSquareNomd, "square-chart-nomd");
    setCssProperty(ui->labelSquarezNomd, "square-chart-znomd");
    setCssProperty(ui->labelNomd, "text-chart-nomd");
    setCssProperty(ui->labelZnomd, "text-chart-znomd");
    ...
    setCssProperty(ui->labelAmountNomd, "text-stake-nomd-disable");
    setCssProperty(ui->labelAmountZnomd, "text-stake-znomd-disable");
...
bool hasCharts = false;
    ...
    if (window)
        connect(window, &NOMADICGUI::windowResizeEvent, this, &DashboardWidget::windowResizeEvent);
#endif
...
void DashboardWidget::loadWalletModel()
{
    ...
    // update the display unit, to not use the default ("NOMD")
    updateDisplayUnit();
}
...
const QMap<int, std::pair<qint64, qint64>> DashboardWidget::getAmountBy()
{
    ...
    for (int i = 0; i < size; ++i) {
        ...
        bool isNomd = modelIndex.data(TransactionTableModel::TypeRole).toInt() != TransactionRecord::StakeZNOMD;
...
bool DashboardWidget::loadChartData(bool withMonthNames)
{
    ...
    chartData->amountsByCache = getAmountBy(); // pair NOMD, zNOMD
    ...
    for (int j = range.first; j < range.second; j++) {
        ...
        if (chartData->amountsByCache.contains(num)) {
            ...
            nomd = (pair.first != 0) ? pair.first / 100000000 : 0;
            znomd = (pair.second != 0) ? pair.second / 100000000 : 0;
            chartData->totalNomd += pair.first;
            chartData->totalZnomd += pair.second;
        }

        chartData->xLabels << ((withMonthNames) ? monthsNames[num - 1] : QString::number(num));

        chartData->valuesNomd.append(nomd);
        chartData->valueszNomd.append(znomd);

        int max = std::max(nomd, znomd);
...
void DashboardWidget::onChartRefreshed()
{
    ...
    set0->setColor(QColor(103,0,103));
    ...
    set0->append(chartData->valuesNomd);
    set1->append(chartData->valueszNomd);

    // Total
    nDisplayUnit = walletModel->getOptionsModel()->getDisplayUnit();
    if (chartData->totalNomd > 0 || chartData->totalZnomd > 0) {
        setCssProperty(ui->labelAmountNomd, "text-stake-nomd");
        setCssProperty(ui->labelAmountZnomd, "text-stake-znomd");
    } else {
        setCssProperty(ui->labelAmountNomd, "text-stake-nomd-disable");
        setCssProperty(ui->labelAmountZnomd, "text-stake-znomd-disable");
    }
    forceUpdateStyle({ui->labelAmountNomd, ui->labelAmountZnomd});
    ui->labelAmountNomd->setText(GUIUtil::formatBalance(chartData->totalNomd, nDisplayUnit));
    ui->labelAmountZnomd->setText(GUIUtil::formatBalance(chartData->totalZnomd, nDisplayUnit, true));
