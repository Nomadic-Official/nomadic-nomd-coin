...
#include "qt/nomadic/send.h"
#include "qt/nomadic/forms/ui_send.h"
#include "qt/nomadic/addnewcontactdialog.h"
#include "qt/nomadic/qtutils.h"
#include "qt/nomadic/sendchangeaddressdialog.h"
#include "qt/nomadic/optionbutton.h"
#include "qt/nomadic/sendconfirmdialog.h"
#include "qt/nomadic/guitransactionsutils.h"
#include "qt/nomadic/loadingdialog.h"
...
SendWidget::SendWidget(NOMADICGUI* parent) :
    PWidget(parent),
    ui(new Ui::send),
    coinIcon(new QPushButton())
{
    ...
    setCssProperty(coinIcon, "coin-icon-nomd");
    ...
    // Connect
    connect(ui->pushLeft, &QPushButton::clicked, [this](){onNOMDSelected(true);});
    connect(ui->pushRight,  &QPushButton::clicked, [this](){onNOMDSelected(false);});
...
void SendWidget::showHideCheckBoxDelegations()
{
    ...
    // coincontrol is not selected, and we are trying to spend transparent NOMDs.
...
void SendWidget::onShieldCoinsClicked()
{
    ...
    if (walletModel && availableBalance > 0) {
        ...
        ProcessSend(recipients, true, [this](QList<SendCoinsRecipient>& recipients) {
            ...
            if (!res.result) {
                inform(tr("Error generating address to shield NOMDs"));
                return false;
            }
        ...
        inform(tr("You don't have any transparent NOMDs to shield."));
    }
}
...
void SendWidget::onNOMDSelected(bool _isTransparent)
{
...
void SendWidget::onContactMultiClicked()
{
    if (focusedEntry) {
        ...
        auto nomdAdd = Standard::DecodeDestination(address.toStdString(), isStakingAddr);

        if (!Standard::IsValidDestination(nomdAdd) || isStakingAddr) {
            inform(tr("Invalid address"));
            return;
        }

        if (walletModel->isMine(nomdAdd)) {
        ...
        if (dialog->res) {
            ...
            if (walletModel->updateAddressBookLabels(nomdAdd, dialog->getLabel().toStdString(),
                    AddressBook::AddressBookPurpose::SEND)) {
